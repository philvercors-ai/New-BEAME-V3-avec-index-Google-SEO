...
interface Artwork {
  id: number;
  slug: string; // Ajouté pour les URLs propres
  title: string;
  category: string;
  image: string;
  technique: string;
...
const ARTWORKS: Artwork[] = [
  ...
  { id: 4, slug: "le-chant-des-cigales", title: "Le chant des cigales", category: "paysage", image: "/images/Le-chant-des-cigales.webp", technique: "Huile", support: "toile de lin", description: "Évocation de la chaleur provençale et du paysage sonore du midi.", price: "750€", dimensions: "50x70cm" },
  { id: 5, slug: "mer-emeraude", title: "Mer Émeraude", category: "mer & océan", image: "/images/mer-emeraude.webp", technique: "Huile", support: "toile de lin", description: "L'éclat cristallin d'un rivage sauvage, capturant le mouvement de l'eau translucide.", price: "890€", dimensions: "65x85cm"},
];

// --- COMPOSANTS SEO ---
const SEO = ({ title, description, image = "/images/o.jpg", type = "website" }: any) => (
  <Helmet>
    <title>{title}</title>
      ...
      <div className="hidden md:flex space-x-8">
        {[
          ...
          <NavLink
            ...
            to={link.path}
            className={({ isActive }) => 
              `${isActive ? "text-amber-700 font-semibold border-b-2 border-amber-700" : "text-gray-500"} hover:text-amber-600 transition capitalize text-[10px] tracking-widest px-1`
            }
          >
            {link.name}
          </NavLink>
...
const AccueilPage = () => (
  <div className="relative h-screen flex items-center justify-center overflow-hidden">
    <SEO 
      title="BÉAME | Artiste Peintre à Vallon Pont d'Arc, Ardèche" 
      description="Découvrez l'univers de BÉAME, artiste peintre à Vallon Pont d'Arc. Galerie de peintures abstraites, paysages de l'Ardèche et marines à l'huile et acrylique."
    />
    <div className="absolute inset-0 z-0">
      <img src="/images/o.jpg" alt="Peinture abstraite BÉAME" className="w-full h-full object-cover scale-105 animate-slow-zoom" />
    ...
    <div className="relative z-10 text-center px-4">
      ...
      <div className="flex flex-col md:flex-row gap-6 justify-center items-center">
        <Link to="/galerie" className="group flex items-center gap-3 px-10 py-4 bg-gray-900 text-white uppercase tracking-widest text-[10px] font-bold hover:bg-amber-800 transition shadow-2xl">
...
const GaleriePage = () => {
  ...
  return (
    <div className="pt-32 pb-20 max-w-7xl mx-auto px-4">
      <SEO title="Galerie d'Art | BÉAME" description="Explorez la galerie de peintures de BÉAME : paysages ardéchois, abstractions et marines." />
          ...
          <Link to={`/galerie/${art.slug}`} key={art.id} className="group">
            <article className="cursor-pointer">
              <div className="relative overflow-hidden aspect-[4/5] bg-gray-100 shadow-md">
                <img src={art.image} alt={`${art.title} - Peinture par BÉAME`} className="w-full h-full object-cover transform group-hover:scale-105 transition duration-700" />
...
const InfoImage = () => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-white flex flex-col md:flex-row overflow-y-auto md:overflow-hidden">
      <SEO 
        title={`${artwork.title} | BÉAME Artiste`} 
        description={`${artwork.title} : ${artwork.technique} sur ${artwork.support}. ${artwork.description}`}
        image={artwork.image}
        type="article"
      />
      <script type="application/ld+json">{JSON.stringify(jsonLd)}</script>

      <Link to="/galerie" className="absolute top-6 left-6 z-[110] flex items-center space-x-2 bg-white/80 backdrop-blur px-4 py-2 rounded-full shadow-lg hover:text-amber-700 transition">
        <ArrowLeft size={18} />
      ...
      <div className="w-full md:w-1/2 h-auto md:h-full flex items-center justify-center p-8 md:p-16 bg-white overflow-y-auto">
        <div className="max-w-md w-full space-y-8">
          ...
          <div className="space-y-4 text-gray-700 text-sm leading-relaxed border-t pt-6">
            <p className="font-serif italic text-lg text-amber-800">« Le geste et la lumière »</p>
          ...
          <Link 
            to={`/contact?sujet=${encodeURIComponent(artwork.title)}`}
            className="block text-center w-full bg-gray-900 text-white py-5 font-bold uppercase tracking-widest text-xs hover:bg-amber-800 transition"
          >
...
const BioPage = () => (
  <div className="pt-32 pb-20">
    <SEO title="Biographie de l'artiste | BÉAME" description="Découvrez le parcours de BÉAME, peintre installé à Vallon Pont d'Arc." />
      ...
      <div className="grid md:grid-cols-5 gap-12 items-center">
        ...
        <div className="md:col-span-2">
          <div className="aspect-[3/4] bg-gray-100 overflow-hidden shadow-2xl rounded-sm">
            <img src="/images/bea.webp" alt="L'artiste peintre BÉAME dans son atelier" className="w-full h-full object-cover grayscale" />
          </div>
        </div>
      </div>
    </div>
...
const ContactPage = () => {
  ...
  return (
    <div className="pt-32 pb-20 bg-white">
      <SEO title="Contact | BÉAME Artiste" description="Contactez BÉAME pour l'acquisition d'une œuvre ou une visite d'atelier à Vallon Pont d'Arc." />
      <div className="max-w-5xl mx-auto px-4 flex flex-col md:flex-row gap-16">
        ...
        <div className="md:w-2/3 bg-gray-50 p-8 md:p-12 shadow-inner">
          {status === "success" ? (
            <div className="text-center py-12 font-serif text-xl">✓ Message transmis.<br/><span className="text-sm font-sans text-gray-400">BÉAME vous répondra sous peu.</span></div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-8">
              <input defaultValue={sujetPredefini ? `Acquisition : ${sujetPredefini}` : ""} placeholder="SUJET" className="w-full bg-transparent border-b border-gray-300 py-3 outline-none focus:border-amber-700 transition text-[10px] tracking-widest font-bold" />
              ...
              <button className="bg-gray-900 text-white px-12 py-4 uppercase tracking-widest text-[10px] font-bold hover:bg-amber-800 transition w-full md:w-auto">Envoyer la demande</button>
            </form>
          )}
        </div>
      </div>
        ...
        <div className="flex flex-col min-h-screen bg-white">
          ...
          <footer className="bg-gray-900 text-white py-12 text-center">
            <p className="text-gray-500 text-[10px] uppercase tracking-[0.2em]">
              © 2025 BÉAME - Artiste Peintre Vallon Pont d'Arc - Tous droits réservés
            </p>
          </footer>
        </div>
      </Router>
